...
Please try with a registered Aadhaar number.`);
  ...
  if (loading) return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="text-center">
        <div className="text-lg font-semibold text-green-900 mb-4">{t("user.dashboard.loading_title")}</div>
        <div className="text-sm text-gray-600">{t("user.dashboard.loading_subtitle", { aadhaar })}</div>
      </div>
    </div>
  );
  ...
  if (error) return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="text-center">
        <div className="text-lg font-semibold text-red-600 mb-4">{t("user.dashboard.error_title")}</div>
        ...
        <div className="text-xs text-gray-500">
          ...
          {error.includes("log in") && (
            <div className="mt-4">
              ...
              >
                {t("user.dashboard.error_retry_login")}
              </a>
            </div>
          )}
          {error.includes("Consumer not found") && (
            <p>{t("user.dashboard.error_test_hint")}</p>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      <h1 className="text-3xl font-bold text-green-900 mb-6 flex items-center gap-2">
        <User className="h-7 w-7 text-green-700" /> {t("user.dashboard.title")}
      </h1>
