                    ...
                    {request.status === 'pending' && (
                      <div className="mt-4 pt-4 border-t border-gray-200">
                        <div className="flex flex-col sm:flex-row gap-3 items-center">
                          ...
                          >
                            <option value="">Select Shopkeeper to Assign</option>
                            {shopkeepers.map(sk => (
                              <option key={sk.walletAddress} value={sk.walletAddress}>
                                {sk.name} - {sk.area} ({sk.walletAddress.slice(0, 6)}...{sk.walletAddress.slice(-4)})
                              </option>
                            ))}
                          </select>
                          <button
                            onClick={() => handleAction(request._id, 'approve')}
                            className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-md transition-colors flex-1 sm:flex-none disabled:bg-gray-400"
                            disabled={assigning || !selectedShopkeepers[request._id]}
                          >
